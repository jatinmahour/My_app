            ...
            [
                ...
                dbc.Col(
                    dbc.Button(
                        ...
                        target="_blank",
                        color="dark",
                        style={"font-family": "Times New Roman", 'margin-left': '550px','margin-top': '10px',
                               "width": 60}
                    )
                ),
        ...
        return html.Div([
            dbc.Row([
                html.H5("Predict Leaf area (m2) : Multiple Linear Regression"
                        , style={"font-family": "Times New Roman", 'padding': 10, "font-weight": "bold",'marginTop': 10}),
                html.H5("The total leaf area of a tree is a difficult thing to measure, it could "
                        "be a destructive method to pull all the leaves off a tree, measure them individually, "
                        "then add the areas together, or a non-destructive digital imaging method. This is a statistical analysis"
                        " to estimate the leaf area of a tree from its dbh(Diameter at breast height), tree height and Age."
                        , style={"font-family": "Times New Roman", 'padding': 5}),
                dbc.Col([
...
def update_graph(value_chosen):
    ...
    fig.update_layout(title="Count of distinct tree species in the selected city.",
                      showlegend=False, xaxis_showticklabels=True, title_font_family="Times New Roman")
    return fig
...
def update_graph(value_chosen):
    ...
    fig.update_layout(title="Count of selected tree species in each city.",
                      showlegend=False,title_font_family="Times New Roman")
    return fig
